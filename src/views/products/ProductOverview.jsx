...
const ProductOverview = () => {
  ...
  return (
    <div className="bg-[#002c5c] text-white">
      ...
      {/* Product Cards */}
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-8 mx-auto px-6 lg:px-44 pb-12">
        {products.map((product, index) => (
          <div
            key={index}
          ...
          >
            {/* Image Section */}
            <div className="relative flex-shrink-0 lg:w-[30%] w-full h-[200px] lg:h-[300px] lg:flex hidden items-center justify-center bg-[#d9e9f8]">
              <img
                src={product.image}
                alt={product.title}
            ...
            {/* Text Section */}
            <div className="relative w-full lg:w-[70%] p-6 lg:py-8 lg:pr-32 inline-table align-middle text-[#00223e]">
              <h2 className="text-[24px] lg:text-[42px] font-extrabold mb-4 uppercase tracking-wider">
                {product.title}
              </h2>
                <a
                  href="#"
                  className="text-sm lg:text-start font-semibold hover:underline transition"
                >
                  {product.description}
                </a>
